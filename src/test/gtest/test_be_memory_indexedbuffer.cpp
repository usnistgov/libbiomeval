...
{
	std::unique_ptr<BE::Memory::IndexedBuffer> ib;

	/* Fixed buffer constructor */
	uint8_t *buf1 = new uint8_t[bufSize];
	ASSERT_NO_THROW(ib.reset(new BE::Memory::IndexedBuffer(buf1, bufSize)));
	...
	ASSERT_NO_THROW(ib.reset(new BE::Memory::IndexedBuffer(buf2)));
	EXPECT_EQ(bufSize, ib->getSize());
	EXPECT_EQ(0, ib->getIndex());
	EXPECT_EQ(&(*buf2), ib->get());

	/* Copy constructor */
	BE::Memory::IndexedBuffer ib2(*ib);
...
TEST(IndexedBuffer, getSize)
{
	...
	BE::Memory::IndexedBuffer ib1(buf1);
	EXPECT_EQ(ib1.getSize(), bufSize);

	uint8_t *buf2 = new uint8_t[bufSize];
	BE::Memory::IndexedBuffer ib2(buf2, bufSize);
...
TEST(IndexedBuffer, EightBit)
{
	fixedScanTest(0, UINT8_MAX, sizeof(uint8_t));
}

TEST(IndexedBuffer, SixteenBit)
{
