...
BiometricEvaluation::IO::ArchiveRecordStore::Impl::Impl(
    const std::string &pathname,
    const std::string &description) :
    RecordStore::Impl(pathname, description, RecordStore::Kind::Archive)
{
	_dirty = false;
...
void
BiometricEvaluation::IO::ArchiveRecordStore::Impl::open_streams()
    const
{
	struct stat sb;
...
void
BiometricEvaluation::IO::ArchiveRecordStore::Impl::close_streams()
{
...
void
BiometricEvaluation::IO::ArchiveRecordStore::Impl::sync()
    const
{
...
uint64_t
BiometricEvaluation::IO::ArchiveRecordStore::Impl::length(
...
void
BiometricEvaluation::IO::ArchiveRecordStore::Impl::read_manifest()
{
	std::string key;
	std::string linebuf;
	ManifestEntry entry;
...
BiometricEvaluation::IO::ArchiveRecordStore::Impl::read(
    const std::string &key)
    const
{
	if (!validateKeyString(key))
...
void
BiometricEvaluation::IO::ArchiveRecordStore::Impl::insert(
    const std::string &key,
...
void
BiometricEvaluation::IO::ArchiveRecordStore::Impl::write_manifest_entry(
    const std::string &key,
    ManifestEntry entry)
{
	if (_archivefp.is_open() == false) {
...
void
BiometricEvaluation::IO::ArchiveRecordStore::Impl::remove(
    const std::string &key)
{
	if (getMode() == Mode::ReadOnly)
		throw Error::StrategyError("RecordStore was opened read-only");
...
void
BiometricEvaluation::IO::ArchiveRecordStore::Impl::flush(
    const std::string &key)
    const
{
	if (getMode() == Mode::ReadOnly)
...
BiometricEvaluation::IO::ArchiveRecordStore::Impl::i_sequence(
    bool returnData,
    int cursor)
{
...
BiometricEvaluation::IO::RecordStore::Record
BiometricEvaluation::IO::ArchiveRecordStore::Impl::sequence(
    int cursor)
{
...
std::string
BiometricEvaluation::IO::ArchiveRecordStore::Impl::sequenceKey(
    int cursor)
{
...
void 
BiometricEvaluation::IO::ArchiveRecordStore::Impl::setCursorAtKey(
    const std::string &key)
{
	if (!validateKeyString(key))
		throw Error::StrategyError("Invalid key format");
...
void
BiometricEvaluation::IO::ArchiveRecordStore::Impl::efficient_insert(
    ManifestMap &m,
...
void
BiometricEvaluation::IO::ArchiveRecordStore::Impl::vacuum(
    const std::string &pathname)
{
	/* See if vacuuming is necessary */
	std::unique_ptr<IO::ArchiveRecordStore::Impl> oldRS(
	    new IO::ArchiveRecordStore::Impl(pathname, Mode::ReadOnly));
	...
	IO::RecordStore::Impl::mergeRecordStores(newName, description,
	    IO::RecordStore::Kind::Archive, paths);

	/* Delete the original RecordStore, then change the name of temp RS */
	auto newRS = IO::RecordStore::Impl::openRecordStore(
	    newName, Mode::ReadWrite);
	try {
...
void
BiometricEvaluation::IO::ArchiveRecordStore::Impl::move(
    const std::string &pathname)
{
	if (this->getMode() == Mode::ReadOnly)
		throw Error::StrategyError("RecordStore was opened read-only");
...
bool
BiometricEvaluation::IO::ArchiveRecordStore::Impl::needsVacuum()
{
	return (this->_dirty);
}
    ...
    const std::string &pathname)
{
	...
	ArchiveRecordStore::Impl rs {pathname};
	return rs.needsVacuum();
}

bool
BiometricEvaluation::IO::ArchiveRecordStore::Impl::keyExists(
    const ManifestMap::key_type &k)
{
	/* O(1) */
	if (!_dirty)
...
std::string
BiometricEvaluation::IO::ArchiveRecordStore::Impl::getManifestName() const
{
	return (canonicalName(MANIFEST_FILE_NAME));
}

std::string
BiometricEvaluation::IO::ArchiveRecordStore::Impl::getArchiveName() const
{
	return (canonicalName(ARCHIVE_FILE_NAME));
}
