 ...
 * a watchdog timer block. The BEGIN macro sets up the jump block and
 * tells the Watchdog object to start handling the alarm signal. Applications
 * must call setInterval() before invoking the BEGIN macro. 
 *
 ...
 *
 * NOTE: On many systems, the sleep(3) call is implemented using alarm
...
namespace BiometricEvaluation {
	...
	namespace Time {
		...
		public:
			...
			/*
			 * The type of Watchdog, based on either process
			 ...
			 * Parameters:
			 *	type
			 *		The type of timer, based on process
			 *		virtual time, or real time.
			 *
			 * Returns:
			 *      The Watchdog object.
			 ...
			 * The setCanSigJump() and clearCanSigJump() methods
			 * are not meant to be used directly by applications,
			 * which should use the BEGIN_WATCHDOG_BLOCK()
			 * and END_WATCHDOG_BLOCK() macro pair.
			 */

			 /*
			 * Set a flag for the Watchdog object to indicate
			 * that the signal jump block has been initialized.
			 */
			void setCanSigJump();
			 ...
			 * Returns:
			 *      true if a the timer expired, false otherwise.
			 */
			bool expired();
