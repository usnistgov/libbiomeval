...
\chapter{View}
\label{chp-view}
Within the \lname\ a \class{View} represents all the information that was derived from
...
a biometric record, along with the derived information. A \class{View} is used to
represent these records as well.

\class{View} objects are created with information taken from a biometric data record,
an ANSI/NIST 2007 file, for example. Most record formats contain information
about the image itself, such as the resolution and size. The \class{View} object can
...
values taken from the \class{Image} object (when available) to those taken from the
\class{View} object.

In the case where a raw image is part of the biometric record, the \class{View} object's
related \class{Image} object will have identical size, resolution, etc. values because
the \class{View} class sets the Image attributes directly. For other image types
(e.g. \class{JPEG}) the \class{Image} object will return attribute values taken from the image
data.
...
contained within the \namespace{Finger} package. See~\chpref{chp-finger} for more
information
